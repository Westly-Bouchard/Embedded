...
# 1 "./mcc_generated_files/system/system.h" 1
# 40 "./mcc_generated_files/system/../uart/eusart2.h" 2
...
# 264 "./mcc_generated_files/system/../uart/eusart2.h"
_Bool EUSART2_IsRxReady(void);
# 273 "./mcc_generated_files/system/../uart/eusart2.h"
...
# 300 "./mcc_generated_files/system/../uart/eusart2.h"
uint8_t EUSART2_Read(void);
# 310 "./mcc_generated_files/system/../uart/eusart2.h"
...
# 335 "./mcc_generated_files/system/../uart/eusart2.h"
...
# 360 "./mcc_generated_files/system/../uart/eusart2.h"
...
void writeToMessageBuffer();
void transmitBuffer();
char IRrecieveBuffer[50];
        ...
        if (EUSART1_IsRxReady()) {
            ...
            switch (cmd) {
                ...
                case 'S':
                    ...
                    mask = 0b00000001;
                    while (mask != 0) {
                        if ((letter & mask) != 0) ECCP2_LoadDutyValue(0);
                        else ECCP2_LoadDutyValue(25);
                        mask = mask << 1;
                        TMR1_CounterSet(0x10000 - bitPeriod[baudRateSelected]);
                        PIR1bits.TMR1IF = 0;
                        while (TMR1_OverflowStatusGet() == 0);
...
void transmitBuffer() {
    ...
    for(uint16_t i = 0; i < 50; i++) {
        transmitChar(messageBuffer[i]);
        printf("%c",messageBuffer[i]);
        if(lastCall == 1) {
            printf("\n\rChecksum: %d", messageBuffer[i]);
            break;
        }
...
void writeToMessageBuffer() {
    ...
    for(uint8_t i = 0; i< 50 -1; i++) {
        while(!EUSART1_IsRxReady());
        char letter = EUSART1_Read();
        if(letter == '\r') {
            break;
        }
        messageBuffer[i] = letter;
        checksum += (uint16_t)letter;
        printf("%c", letter);
        persist = i;
    }
    messageBuffer[persist+1] = '\0';
    printf("Checksum: %d", checksum);
    messageBuffer[persist+2] = checksum;
}
...
void myEUSART2ISR() {
    ...
    static enum states rxState = IDLE;


    if(rxState == IDLE) {
        IRrecieveBuffer[0] = RCREG2;
        rxState = MIDDLE_OF_RX;
        rxBufIDX++;
        receiveBusy = 1;
    } else if (rxState == MIDDLE_OF_RX) {
